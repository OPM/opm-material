...
{
    typedef Opm::DenseAd::EvaluationV<Scalar> Eval;

    // test the constructors of the Opm::DenseAd::Evaluation class
    const Scalar c = 1.234;
    const Scalar x = 4.567;
    const Scalar y = 8.910;
    const Eval cEval = Eval::createConstant(c, numVars);
    // const Eval c2Eval OPM_UNUSED = c;
    const Eval xEval = Eval::createVariable(x, 0, numVars);
    const Eval yEval = Eval::createVariable(y, 1, numVars);
    ...
    {
        Eval a = Eval::createConstant(1.0, numVars);
        Eval b = Eval::createConstant(2.0, numVars);
        if (a >= b)
            throw std::logic_error("oops: operator>=");
        if (a >= 2.0)
            throw std::logic_error("oops: operator>=");
...
void test1DFunction(AdFn* adFn, ClassicFn* classicFn, Scalar xMin = 1e-6, Scalar xMax = 1000)
{
    ...
    for (int i = 0; i < n; ++ i) {
        ...
        const auto& xEval = Eval::createVariable(x, 0, numVars);
...
void test2DFunction1(AdFn* adFn, ClassicFn* classicFn, Scalar xMin, Scalar xMax, Scalar y)
{
    ...
    for (int i = 0; i < n; ++ i) {
        ...
        const auto& xEval = Eval::createVariable(x, 0, numVars);
        const auto& yEval = Eval::createConstant(y, numVars);
...
void test2DFunction2(AdFn* adFn, ClassicFn* classicFn, Scalar x, Scalar yMin, Scalar yMax)
{
    ...
    for (int i = 0; i < n; ++ i) {
        ...
        const auto& xEval = Eval::createConstant(x, numVars);
        const auto& yEval = Eval::createVariable(y, 1, numVars);
        const Eval& zEval = adFn(xEval, yEval);
...
void testPowBase(Scalar baseMin = 1e-2, Scalar baseMax = 100)
{
    ...
    Scalar exp = 1.234;
    const auto& expEval = Eval::createConstant(exp, numVars);

    int n = 100*1000;
    for (int i = 0; i < n; ++ i) {
        Scalar base = Scalar(i)/(n - 1)*(baseMax - baseMin) + baseMin;

        const auto& baseEval = Eval::createVariable(base, 0, numVars);
...
void testPowExp(Scalar expMin = -100, Scalar expMax = 100)
{
    ...
    Scalar base = 1.234;
    const auto& baseEval = Eval::createConstant(base, numVars);

    int n = 100*1000;
    for (int i = 0; i < n; ++ i) {
        Scalar exp = Scalar(i)/(n - 1)*(expMax - expMin) + expMin;
        const auto& expEval = Eval::createVariable(exp, 1, numVars);
...
void testAtan2()
{
    ...
    for (int i = 1; i < n; ++ i) {
        ...
        for (int j = 1; j < n; ++ j) {
            ...
            const Eval& yEval = Eval::createVariable(y, 0, numVars);
...
inline void testAll()
{
    ...
    // TODO: there minght be some problem here, since we do not have the information about the numVars here.
    test2DFunction1(Opm::DenseAd::min<Scalar>,
                            myScalarMin,
                            -1000, 1000,
                            /*p=*/1.234);

    test2DFunction2(Opm::DenseAd::min<Scalar>,
                            myScalarMin,
                            /*T=*/1.234,
                            -1000, 1000);

    std::cout << "testing max()\n";
    test2DFunction1(Opm::DenseAd::max<Scalar>,
                            myScalarMax,
                            -1000, 1000,
                            /*p=*/1.234);

    test2DFunction2(Opm::DenseAd::max<Scalar>,
                            myScalarMax,
                            /*T=*/1.234,
                            -1000, 1000);
    ...
    test1DFunction(Opm::DenseAd::exp<Scalar>,
                           static_cast<Scalar (*)(Scalar)>(std::exp),
                           -100, 100);

    std::cout << "testing log()\n";
    test1DFunction(Opm::DenseAd::log<Scalar>,
    ...
    // while (true) {
        ...
        typedef Opm::DenseAd::EvaluationV<Scalar> TmpEval;
        TmpEval eval1 = TmpEval::createConstant(1.0, numVars);
        TmpEval eval2 = TmpEval::createConstant(2.0, numVars);
        ...
        resultEval = Opm::constant<TmpEval, numVars>(val1);
        resultEval = Opm::variable<TmpEval, numVars>(val1, /*idx=*/0);
