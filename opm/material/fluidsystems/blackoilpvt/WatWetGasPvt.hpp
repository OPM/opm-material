...
/*!
 * \file
 * \copydoc Opm::WatWetGasPvt
 */
#ifndef OPM_WAT_WET_GAS_PVT_HPP
#define OPM_WAT_WET_GAS_PVT_HPP
...
public:
    ...
    WatWetGasPvt(const std::vector<Scalar>& gasReferenceDensity,
              const std::vector<Scalar>& watReferenceDensity,
              ...
              const std::vector<TabulatedOneDFunction>& saturatedWatVaporizationFactorTable,
              const std::vector<TabulatedOneDFunction>& saturationPressure,
              Scalar vapPar1)
        : gasReferenceDensity_(gasReferenceDensity)
        , watReferenceDensity_(watReferenceDensity)
        ...
        , saturatedWatVaporizationFactorTable_(saturatedWatVaporizationFactorTable)
...
#if HAVE_ECL_INPUT
    /*!
     * \brief Initialize the parameters for water wet gas using an ECL deck.
     *
    ...
    void initFromState(const EclipseState& eclState, const Schedule& schedule)
    {
        ...
        for (unsigned regionIdx = 0; regionIdx < numRegions; ++ regionIdx) {
            ...
            auto& watVaporizationFac = saturatedWatVaporizationFactorTable_[regionIdx];

            watVaporizationFac.setXYArrays(saturatedTable.numRows(),
            ...
            // extract the table for the gas dissolution and the wat formation volume factors
            for (unsigned outerIdx = 0; outerIdx < saturatedTable.numRows(); ++ outerIdx) {
                ...
                for (size_t innerIdx = 0; innerIdx < numRows; ++ innerIdx) {
                    Scalar Rw = underSaturatedTable.get("RV" , innerIdx);
...
private:
    ...
    {
        std::vector<double> RwArray = curTable.getColumn("RV").vectorCopy();
        ...
        for (size_t newRowIdx = 1; newRowIdx < masterTable.numRows(); ++ newRowIdx) {
            const auto& RWColumn = masterTable.getColumn("RV");
...
public:
    ...
    {
        watReferenceDensity_.resize(numRegions);
        ...
        saturatedWatVaporizationFactorTable_.resize(numRegions);
    ...
    void setReferenceDensities(unsigned regionIdx,
                               ...
                               Scalar /*rhoRefWater*/)
    {
        watReferenceDensity_[regionIdx] = rhoRefWat;
        gasReferenceDensity_[regionIdx] = rhoRefGas;
    }

    /*!
     * \brief Initialize the function for the wat vaporization factor \f$R_w\f$
     *
     * \param samplePoints A container of (x,y) values.
     */
    void setSaturatedGasWatVaporizationFactor(unsigned regionIdx, const SamplingPoints& samplePoints)
    { saturatedWatVaporizationFactorTable_[regionIdx].setContainerOfTuples(samplePoints); }
     ...
     * represents the partial density of the wat component in the gas phase at a given
     * pressure. This method only requires the volume factor of wat-saturated gas (which
     * only depends on pressure) while the dependence on the wat mass fraction is
     * guesstimated...
     */
    void setSaturatedGasFormationVolumeFactor(unsigned regionIdx, const SamplingPoints& samplePoints)
    {
        ...
        const auto& RwTable = saturatedWatVaporizationFactorTable_[regionIdx];

        Scalar T = 273.15 + 15.56; // [K]

        Scalar RwMin = 0.0;
        Scalar RwMax = RwTable.eval(saturatedWatVaporizationFactorTable_[regionIdx].xMax(), /*extrapolate=*/true);
        ...
        Scalar rhooRef = watReferenceDensity_[regionIdx];
        ...
        // fraction. note that this assumes wat of constant compressibility. (having said
        ...
        for (size_t RwIdx = 0; RwIdx < nRw; ++RwIdx) {
            ...
            invGasB.appendXPos(Rw);

            for (size_t pIdx = 0; pIdx < nP; ++pIdx) {
                Scalar pg = poMin + (poMax - poMin)*pIdx/nP;

                Scalar poSat = saturationPressure(regionIdx, T, Rw);
                ...
                invGasB.appendSamplePoint(RwIdx, pg, 1.0/Bg);
            }
        }
    }
     ...
     * the wat component in the gas phase, this function depends on the gas dissolution
    ...
    /*!
     * \brief Initialize the phase viscosity for wat saturated gas
     *
     * The gas viscosity is a function of \f$(p_g, X_g^O)\f$, but this method only
     * requires the viscosity of wat-saturated gas (which only depends on pressure) while
    ...
    void setSaturatedGasViscosity(unsigned regionIdx, const SamplingPoints& samplePoints  )
    {
        ...
        Scalar RwMin = 0.0;
        Scalar RwMax = watVaporizationFac.eval(saturatedWatVaporizationFactorTable_[regionIdx].xMax(), /*extrapolate=*/true);
    ...
    Evaluation internalEnergy(unsigned regionIdx OPM_UNUSED,
                        ...
                        const Evaluation& w OPM_UNUSED) const
    {
    ...
    /*!
     * \brief Returns the dynamic viscosity [Pa s] of wat saturated gas at a given pressure.
     */
    template <class Evaluation>
    ...
    /*!
     * \brief Returns the formation volume factor [-] of wat saturated gas at a given pressure.
     */
    template <class Evaluation>
    ...
    /*!
     * \brief Returns the wat vaporization factor \f$R_v\f$ [m^3/m^3] of the gas phase.
     */
    template <class Evaluation>
    Evaluation saturatedWatVaporizationFactor(unsigned regionIdx,
                                              const Evaluation& /*temperature*/,
                                              const Evaluation& pressure) const
    {
        return saturatedWatVaporizationFactorTable_[regionIdx].eval(pressure, /*extrapolate=*/true);
    }

    /*!
     * \brief Returns the wat vaporization factor \f$R_w\f$ [m^3/m^3] of the gas phase.
     *
     * This variant of the method prevents all the wat to be vaporized even if the gas
    ...
    Evaluation saturatedWatVaporizationFactor(unsigned regionIdx,
                                              const Evaluation& /*temperature*/,
                                              const Evaluation& pressure,
                                              const Evaluation& watSaturation,
                                              Evaluation maxWatSaturation) const
    {
        Evaluation tmp =
            saturatedWatVaporizationFactorTable_[regionIdx].eval(pressure, /*extrapolate=*/true);

        // apply the vaporization parameters for the gas phase (cf. the Eclipse VAPPARS
        // keyword)
        maxWatSaturation = Opm::min(maxWatSaturation, Scalar(1.0));
        if (vapPar1_ > 0.0 && maxWatSaturation > 0.01 && watSaturation < maxWatSaturation) {
            static const Scalar eps = 0.001;
            const Evaluation& Sw = Opm::max(watSaturation, eps);
            tmp *= Opm::max(1e-3, Opm::pow(Sw/maxWatSaturation, vapPar1_));
        }
     ...
     *        depending on its mass fraction of the wat component
     *
     * This method uses the standard blackoil assumptions: This means that the Rw value
     * does not depend on the saturation of water. (cf. the Eclipse VAPPARS keyword.)
     *
     * \param Rw The surface volume of wat component dissolved in what will yield one
    ...
    const std::vector<Scalar>& watReferenceDensity() const {
        return watReferenceDensity_;
    }
    ...
    const std::vector<TabulatedOneDFunction>& saturatedWatVaporizationFactorTable() const {
        return saturatedWatVaporizationFactorTable_;
    }
    ...
    bool operator==(const WetGasPvt<Scalar>& data) const
    {
        return this->gasReferenceDensity() == data.gasReferenceDensity() &&
               this->watReferenceDensity() == data.watReferenceDensity() &&
               ...
               this->saturatedWatVaporizationFactorTable() == data.saturatedWatVaporizationFactorTable() &&
...
private:
    void updateSaturationPressure_(unsigned regionIdx)
    {
        ...
        const auto& watVaporizationFac = saturatedWatVaporizationFactorTable_[regionIdx];

        // create the taublated function representing saturation pressure depending of
        // RW
        size_t n = watVaporizationFac.numSamples();
        Scalar delta = (watVaporizationFac.xMax() - watVaporizationFac.xMin())/Scalar(n + 1);

        SamplingPoints pSatSamplePoints;
        Scalar Rw = 0;
        for (size_t i = 0; i <= n; ++ i) {
            Scalar pSat = watVaporizationFac.xMin() + Scalar(i)*delta;
            Rw = saturatedWatVaporizationFactor(regionIdx, /*temperature=*/Scalar(1e30), pSat);

            Pair val(Rw, pSat);
            pSatSamplePoints.push_back(val);
        }

        //Prune duplicate Rw values (can occur, and will cause problems in further interpolation)
    ...
    std::vector<Scalar> watReferenceDensity_;
    ...
    std::vector<TabulatedOneDFunction> saturatedWatVaporizationFactorTable_;
